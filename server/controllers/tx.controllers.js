...
const sendTransaction = async (to, amount) => {
  ...
  var transactionData = contract.methods.safeTransferFrom(from, to, id, amount, data).encodeABI(); //Create the data for token transaction.
  var rawTransaction = {"from": serverAddr, "to": contractHx, "gas": 100000, "data": transactionData }; 

  web3.eth.accounts.signTransaction(rawTransaction, process.env.SERVER_PK)
...
const sendZ = async (req, res) => {
    ...
    const transactionData = contract.methods.safeTransferFrom(from, to, id, amount, data).encodeABI(); //Create the data for token transaction.
    const rawTransaction = {"from": serverAddr, "to": contractHx, "gas": 100000, "data": transactionData }; 
